...
def ts_teach_user_study(ax1, ax2, dataset, W_typezero=(7.0, 0.0), W_typeone=(7.0, -7.0), educability=0.01, n_interactions=100,
    ...
    # cost[0,...,n_covars-1] is recommendation cost per covariate. cost[n_covars] is educate cost.
    cost = torch.zeros(n_covars + 1) + 0.05
    cost[-1] = 0.5

    #user response legend
    user_response_legend = ["exclude", "include"]
    ...
    for i in range(n_interactions):
        ...
        strt = 6 + (3 * i)
        endn = strt + i
        included_vars = torch.where(aux_data_dict["xi"])[0].tolist()
        if len(included_vars) == 0: 
            print("No variables currently included in the model")
        else:
            print("You already included the following variables:", ", ".join(["X{}".format(i) for i in included_vars]))
        ...
        if i == 0:
            ...
            system_action_string.append("Recommend: {}".format(act_in))
            ...
            sampled_user_models.append(sample_user_model)
            ###
            ...
            e_or_r, r_i = planning_function(n_covars, aux_data_dict["xi"], corr_mat, sample_user_model, cost,
                                            n_interactions - i + 1, n_training_collinear, n_training_noncollinear,
                                            test_datasets, prev_action=prev_action)
            prev_action = r_i
            ...
            if e_or_r == 0:
                ...
                act_in = -1
                cumulative_cost[i] = cumulative_cost[i - 1] + cost[-1]
                system_action_string.append("Tutor")
        ...
        if act_in != -1:
            if act_in in included_vars:
                print("Would you like to keep variable X{} in the model?".format(act_in))
            else:
                print("Would you like to include variable X{} in the model?".format(act_in))

            X_in = training_X[:, act_in]
            X_name = 'X' + str(act_in)

            d = {X_name: X_in, 'y': training_y}
            df = pandas.DataFrame(data=d)
            #plt.figure("X-Y")
            #plt.clf()
            ax1.cla()         
            sns.scatterplot(ax=ax1, x=X_name, y="y", data=df)
            # plt.show()
            ...
            if masked.size != 0:
                # The maximum absolute cross-correlation to selected vars.
                max_cross_corr = np.max(masked)
                j = [x for x in np.argsort(corr_mat[act_in, :]) if mask[x]][-1]
                d = {X_name: X_in, 'X' + str(j): training_X[:, j]}
                df = pandas.DataFrame(data=d)
                #plt.figure("X-X")
                #plt.clf()
                sns.scatterplot(ax=ax2, x=X_name, y='X' + str(j), data=df)
                # plt.show()
            ...
            user_response_times.append(time.time() - start_time)
            user_responses.append(outcome)
            user_response_string.append(user_response_legend[outcome])
            # print("Variable: {} Outcome: {}".format(act_in, outcome))

            previous_xi_n_collinear = torch.sum(aux_data_dict["xi"][0:n_training_collinear])
            previous_xi_n_noncollinear = torch.sum(aux_data_dict["xi"][n_training_collinear:-1])
            previous_terminal_cost =(np.abs(previous_xi_n_collinear - 1)) + (np.abs(n_training_noncollinear - previous_xi_n_noncollinear))

            if outcome == 1:
                # If accepted, set the variable's inclusion indicator to true.
                aux_data_dict["xi"][act_in] = True
                new_xi_n_collinear = torch.sum(aux_data_dict["xi"][0:n_training_collinear])
                new_xi_n_noncollinear = torch.sum(aux_data_dict["xi"][n_training_collinear:-1])
                new_terminal_cost = (np.abs(new_xi_n_collinear - 1)) + (np.abs(
                n_training_noncollinear - new_xi_n_noncollinear))

                if (new_terminal_cost - previous_terminal_cost) <= 0:
                    is_incorrect_action.append(False)
                else:
                    is_incorrect_action.append(True)
            else:
                # If not, set it to false.
                aux_data_dict["xi"][act_in] = False
                new_xi_n_collinear = torch.sum(aux_data_dict["xi"][0:n_training_collinear])
        ...
        # This is an educate action!
            ...
            #subprocess.Popen(['open','Material/Full_Tutorial.pdf'])
        ...
        model_built_sofar.append(str(torch.where(aux_data_dict["xi"])[0].tolist()))
    # print(fit)
    # arviz.plot_trace(fit)
    # pyplot.show()
